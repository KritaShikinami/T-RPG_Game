import math, random, json, os, shutil, curses
...
def load_player_spawn(spawn_id="start_dock"):
    with open("data/spawn_points.json", "r", encoding="utf-8") as f:
        data = json.load(f)
    for spawn in data["spawns"]:
        if spawn["type"] == "player" and spawn.get("id") == spawn_id:
            return spawn["region"], spawn["x"], spawn["y"]
    raise ValueError(f"Spawn ID '{spawn_id}' bulunamadı.")
...
def boxed(lines: list[str], width: int = 80) -> str:
    wrapped = []
    for line in lines:
    ...
    max_len = min(max(len(l) for l in wrapped), width - 4)
    top = "┌" + "─" * (max_len + 2) + "┐"
    bottom = "└" + "─" * (max_len + 2) + "┘"
    body = [f"│ {l.ljust(max_len)} │" for l in wrapped]
...
# Hareket Sistemi
# =========================
def try_move(region, pos: Tuple[int, int], dx: int, dy: int, stdscr=None):
    nx, ny = pos[0] + dx, pos[1] + dy

    # Harita sınır kontrolü
    if nx < 0 or ny < 0 or nx >= region.width or ny >= region.height:
        return pos
    ...
    for obj in getattr(region, "objects", []):
        ...
        # 1) NPC Etkileşimi
        if obj.get("type") == "npc" and abs(nx - ox) <= 1 and abs(ny - oy) <= 1:
        ...
        # 2) Geçit (Gate) Kontrolü
        if obj.get("type") == "gate" and nx == ox and ny == oy:
            exit_info = obj.get("exit_to")
            if exit_info:
                return ("__region_change__", exit_info["region"], exit_info["x"], exit_info["y"])
...
# NPC Etkileşim
# =========================
def interact_npc(npc_id: str, has_met: bool = False):
    npc = load_npc(npc_id)
    dialog_state = "repeat" if has_met else "first_time"
    dialog = npc["dialog"][dialog_state]
    responses = npc["dialog"]["responses"]

    # Yüzü göster
    print(center(boxed(npc["ascii_face"])))
    print()

    # Diyalog metni
    print(center(boxed(dialog["line"].split("\n"))))
    print()

    # Seçenekler
    options = dialog["options"]
    print(center("  ".join(f"[ {opt} ]" for opt in options)))
    print()
    ...
    while True:
        ...
        if response is None:
            print(center(boxed(["Anlamadım..."])))
            break

        if isinstance(response, str):
            print("\n" + center(boxed(response.split("\n"))))
            break

        elif isinstance(response, dict):
            print("\n" + center(boxed(response["line"].split("\n"))))
            print()
            print(center("  ".join(f"[ {opt} ]" for opt in response["options"])))
            print()
            choice = input("Seçimin: ").strip()

# --- VIEWPORT ---

def render_viewport(region: Region, player_pos: Tuple[int, int], vw: int = 30, vh: int = 11) -> List[str]:
    px, py = player_pos
    half_w, half_h = vw // 2, vh // 2
    lines = []

    for y in range(py - half_h, py + half_h + 1):
        row = []
        for x in range(px - half_w, px + half_w + 1):
            if not region.in_bounds(x, y):
                row.append("░")  # harita dışı
            elif x == px and y == py:
                row.append("@")  # oyuncu
            else:
                row.append(region.grid[y][x].char)
...
def draw_screen(region: Region, player_pos: Tuple[int, int]):
    clear()
    viewport = render_viewport(region, player_pos)
    panel = describe_tile(region, player_pos)

    # Harita üstte, panel altta
    print(center(boxed(viewport)))
    print()
    print(center(boxed(panel)))
